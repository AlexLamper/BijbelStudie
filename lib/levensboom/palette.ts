...
import { DEFAULT_SCENE, sceneSpec, type SceneId } from './scenes';
import { DEFAULT_SPECIES, speciesParams, type SpeciesId } from './species';
...
const FOLIAGE: Record<Season, { leaf: string; leafAlt: string; blossom: string | null; ground: string }> = {
  spring: { leaf: '#6FBF73', leafAlt: '#8FD694', blossom: '#F7B8CE', ground: '#4E7C43' },
  summer: { leaf: '#3F8F4F', leafAlt: '#57A862', blossom: '#F2A2C0', ground: '#43703C' },
  autumn: { leaf: '#C9772E', leafAlt: '#E0A03C', blossom: null, ground: '#6B5A32' },
  winter: { leaf: '#7D8B7A', leafAlt: '#9AA79A', blossom: null, ground: '#5B6660' },
};
...
export function buildPalette(
  ...
  // Always lower-case hex out, whether or not a mix touched the colour, so a
  // renderer key or a test never sees the same colour spelled two ways.
  const desaturate = (color: string) => (night ? mix(color, NIGHT_MIX, 0.25) : color.toLowerCase());
  ...
  let leafAlt = foliage.leafAlt;
  if (sp.leaf && sp.leafAlt) {
    const turning = season === 'autumn' || season === 'winter';
    if (!turning) {
      leaf = sp.leaf;
      leafAlt = sp.leafAlt;
    } else if (sp.evergreen) {
      const dull = season === 'winter' ? 0.3 : 0.1;
      leaf = mix(sp.leaf, WINTER_MIX, dull);
      leafAlt = mix(sp.leafAlt, WINTER_MIX, dull);
    }
  }
  ...
  return {
    ...
    blossom: blossom ? desaturate(blossom) : null,
    fruit: desaturate(sp.fruit),
    fruitAlt: desaturate(sp.fruitAlt),
    ground: desaturate(ground),
    groundDeep: desaturate(groundDeep),
    far: desaturate(scene.far),
    farAlt: desaturate(scene.farAlt),
    water: scene.water ? desaturate(scene.water) : null,
    accent: scene.accent.toLowerCase(),
    night,
  };
}
