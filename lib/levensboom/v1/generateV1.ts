/**
 * The v1 generator, frozen. Used only by the design tooling (`npm run tree:sheet`
 * baseline) and by `npm run tree:calibrate` to build `LEGACY_EQUIV` - the table
 * that keeps existing accounts' trees from shrinking (plan §5.4). Never imported
 * by app code. Delete once the table is final; git history keeps it.
 */
...
type Ornament = { x: number; y: number; size: number; index: number };
 ...
 */
export function growthForLevelV1(level: number): number {
  return 1 - 1 / (1 + (Math.max(1, level) - 1) / 6);
}

/** 0 at level 1 (the stem only), 1 at level 2, 4 at level 8, 6 from level 26. */
export function maxDepthForLevelV1(level: number): number {
  return Math.min(7, Math.round(7 * growthForLevelV1(level)));
}

export function generateTreeV1(input: TreeInputV1): TreeSceneV1 {
  ...
  const growth = growthForLevelV1(level);
  const maxDepth = maxDepthForLevelV1(level);
  ...
  const recurse = (
    ...
    for (let i = 0; i < childCount; i += 1) {
      ...
      const droop = (droopT + sp.droopBase) * ((depth + 1) / maxDepth);
