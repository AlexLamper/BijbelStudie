...
import { ROUTE_LABELS, type RouteKey } from "./analyticsRoutes";
...
/**
 * [days] is clamped, never trusted: the caller supplies it as a query
 * parameter on both the cookie route and the bearer route.
 */
export async function adminInsightsPayload(days: unknown): Promise<AdminPayload> {
  ...
  try {
    return { status: 200, body: await buildInsights(days) };
  } catch (err) {
    console.error("[admin/insights] query failed", err);
    return {
      status: 500,
      body: { error: err instanceof Error ? err.message : "Kon inzichten niet berekenen" },
    };
  }
}

async function buildInsights(days: unknown): Promise<Record<string, unknown>> {
  await connectMongoDB();

  const requested = typeof days === "string" ? parseInt(days, 10) : Number(days);
  const range = Math.min(Math.max(Number.isFinite(requested) ? Math.trunc(requested) : 30, 7), 365);
