...
/** A status plus the JSON body, so both the cookie route and the bearer route
 *  can wrap the same numbers in their own response helper. */
export type AdminPayload = { status: number; body: Record<string, unknown> };

export async function adminStatsPayload(): Promise<AdminPayload> {
  try {
    ...
    return {
      status: 500,
      body: { error: error instanceof Error ? error.message : "Kon statistieken niet berekenen" },
    };
  }
}

async function buildStats(): Promise<AdminPayload> {
  ...
  if (!connection) {
    ...
    return {
      status: 503,
      body: { error: "Geen verbinding met de database. Probeer het over een minuut opnieuw." },
    };
  }
