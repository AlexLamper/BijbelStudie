...
import MidnightAvatar from "./MidnightAvatar"
import { FreeMembershipPanel, ProMembershipPanel, type BillingInfo } from "./MembershipPanel"
import ReviewBar, { type PreviewState } from "./ReviewBar"
...
/**
 * Getoond in het Pro-voorbeeld wanneer het echte account geen Pro heeft, zodat
 * het paneel niet leeg beoordeeld wordt. Nergens vandaan gelezen, nooit
 * teruggeschreven.
 */
...
/**
 * /profiel/versie-a - ontwerp "Midnight".
 *
 * Een kopie van app/profiel/page.tsx met dezelfde fetches en bewerkingen
 * (GET /api/user, PUT /api/user/update, `updateSession()` na hernoemen). Anders
 * is alleen de Pro-weergave: de avatarring en het PRO-merk (./MidnightAvatar),
 * het Pro-chipje in de kop, en het lidmaatschapspaneel (./MembershipPanel) in
 * plaats van de Abonnement-kaart.
 *
 * Eén extra, alleen-lezen request voor het paneel: GET
 * /api/subscription/billing-state (plan, verlengdatum), alleen wanneer het echte
 * account Pro is. Verder responsief: de rail stapelt onder lg.
 */
export default function MidnightProfile() {
  ...
  const isPro = (preview ?? (realPro ? "pro" : "gratis")) === "pro"
  ...
  // In the Pro preview of a free account the admin copy would be wrong too.
  const isAdmin = realPro && !!user?.isAdmin
  const levelDot = Math.round(112 * 0.31)
        ...
        <div className="flex min-h-full flex-col gap-5 lg:flex-row">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            <Card className="flex flex-none flex-col items-start gap-5 p-5 sm:flex-row">
              <MidnightAvatar size={96} pro={isPro} mark streak={levensboom?.streak ?? null} />
          ...
          <aside className="flex w-full flex-none flex-col gap-[13px] lg:w-[326px]">
            <Card className="flex-none p-[15px]">
              ...
              <div className="mt-[13px] flex justify-center">
                <MidnightAvatar size={112} pro={isPro}>
                  <span
                    ...
                    style={{
                      width: levelDot,
                      height: levelDot,
                      fontSize: Math.max(10, Math.round(levelDot * 0.45)),
            ...
            {waiting ? (
              ...
              <ProMembershipPanel
                ...
                billing={realPro ? billing : DEMO_BILLING}
                isAdmin={isAdmin}
              />
            ) : (
